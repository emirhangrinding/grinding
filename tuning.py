...
def optimise_ssd_hyperparams(
    ...
    def _objective(trial):
        ...
        # Use the exact metrics calculated by SSD (no recalculation!)
        current_metrics = {
            'target_digit_acc': ssd_metrics['target_digit_acc'],      # EXACT values from SSD
        ...
        print(
            ...
            f"Target Digit={current_metrics['target_digit_acc']:.4f} (baseline: {BASELINE_METRICS_WITH_MIA['target_digit_acc']:.4f}), "
            f"Other Digit={current_metrics['other_digit_acc']:.4f} (baseline: {BASELINE_METRICS_WITH_MIA['other_digit_acc']:.4f}), "
            f"Target Subset={current_metrics['target_subset_acc']:.4f} (baseline: {BASELINE_METRICS_WITH_MIA['target_subset_acc']:.4f}), "
            f"Other Subset={current_metrics['other_subset_acc']:.4f} (baseline: {BASELINE_METRICS_WITH_MIA['other_subset_acc']:.4f}), "
            f"Test Digit Overall={current_metrics['test_digit_acc']:.4f} (baseline: {BASELINE_METRICS_WITH_MIA['test_digit_acc']:.4f}), "
            f"MIA={current_metrics['mia_score']:.2f}% (baseline: {BASELINE_METRICS_WITH_MIA['mia_score']:.2f}%) | "
            f"Delta Score={delta_score:.4f}"
        )

        # Keep track for analysis
        trial.set_user_attr("target_digit_acc", current_metrics['target_digit_acc'])
        trial.set_user_attr("other_digit_acc", current_metrics['other_digit_acc'])
        trial.set_user_attr("target_subset_acc", current_metrics['target_subset_acc'])
        trial.set_user_attr("other_subset_acc", current_metrics['other_subset_acc'])
        trial.set_user_attr("test_digit_acc", current_metrics['test_digit_acc'])
        trial.set_user_attr("mia_score", current_metrics['mia_score'])
