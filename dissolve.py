...
def dissolve_unlearn_subset(
    ...
    *, dataset_name: str = "CIFAR10", num_clients: int = 10, head_size: str = 'big'):
    ...
    New approach:
    1. Store *randomly initialised* weights (pre-learning)
    2. Zero out forget-sensitive weights (FIM_Df/FIM_Dr > gamma)
    3. Identify retain-sensitive weights (FIM_Dr/FIM_Df > beta)
    4. Reset retain-sensitive weights to their RANDOM initial values
    5. Fine-tune only retain-sensitive weights on retain set
    ...
    print(f"\n--- Starting DISSOLVE Unlearning for Subset ({mode_desc}) ---")   
    unlearned_model = copy.deepcopy(pretrained_model)
    unlearned_model.to(device)

    # Store randomly initialised weights (pre-learning) for later reset
    print("Storing randomly initialised weights (pre-learning)...")

    # Re-instantiate the same architecture to obtain fresh random weights.
    random_init_model = pretrained_model.__class__(
        dataset_name=dataset_name,
        num_clients=num_clients,
        head_size=head_size
    )
    random_init_state = random_init_model.state_dict()

    initial_weights = {}
    for name, param in unlearned_model.named_parameters():
        if name.startswith("resnet.") and name in random_init_state:
            initial_weights[name] = random_init_state[name].clone()
    ...
    if total_forget_sensitive_weights == 0 and total_retain_sensitive_weights == 0:
        ...
        print(f"--- DISSOLVE Unlearning Finished ({mode_desc}) ---")
    ...
    # 5. Reset retain-sensitive weights (W_r) to their RANDOM initial values
    print("Resetting retain-sensitive weights (W_r elements, RESNET ONLY) to random initial values...")
    for name, param in unlearned_model.named_parameters():
        if not name.startswith("resnet."):
            continue
        if name in retain_sensitivity_masks and name in initial_weights:
            # Ensure mask and initial weights are on the same device as the parameter tensor
            mask_wr = retain_sensitivity_masks[name].to(param.device)
            init_weights_device = initial_weights[name].to(param.device)
            if mask_wr.any():
                with torch.no_grad():
                    param.data[mask_wr] = init_weights_device[mask_wr]

    # 6. Set up fine-tuning parameters (retain-sensitive weights in ResNet and, optionally, heads)
    ...
    # 7. Fine-tune (only W_r elements should be updated, only resnet)
    ...
    # DISSOLVE unlearning finished for the chosen parameter subset.
    return unlearned_model 